...
namespace LewzenServer {
    ...
    void Notched::init() {
        ...
        ControlLeft = createCorePoint("ControlLeft",getX()+getWidth()/4,getY()+getHeight()/2);
        ControlRight = createCorePoint("ControlRight",getX()+getWidth()/10*6,getY()+getHeight()/4);
        ...
        // 绑定图形属性
        std::function<const std::string()> _getPath = std::bind(&Notched::getPath, this);
        SVGIPath->D.bind(_getPath);
    }
    // 拷贝
    ComponentAbstract &Notched::operator=(const ComponentAbstract &comp) {
        // 拷贝父类
        Rectangle::operator=(comp);

        auto &p = dynamic_cast<const Notched &>(comp);
        // 拷贝关键点位置
    ...
    // 序列化，并记录已操作的
    void Notched::serialize(json &j, std::vector<std::string> &processed) {
        // 父类序列化
        Rectangle::serialize(j, processed);
    }
    // 反序列化
    ComponentAbstract &Notched::operator=(const json &j) {
        // 父类反序列化
    ...
    //// Basics虚接口
    void Notched::moveCorePoint(const std::string &id, const double &dx, const double &dy) {
    ...
    //// IsometricCube接口
    // 计算路径
    const std::string Notched::getPath() const {
