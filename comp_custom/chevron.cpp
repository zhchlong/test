...
// 设置类型
        ...
        ControlRight = createCorePoint("ControlRight",getX()+getWidth()/4*3,getY());
        ...
        // 绑定图形属性
        std::function<const std::string()> _getPath = std::bind(&Chevron::getPath, this);
        SVGIPath->D.bind(_getPath);
    }
    // 拷贝
    ComponentAbstract &Chevron::operator=(const ComponentAbstract &comp) {
        // 拷贝父类
        Rectangle::operator=(comp);

        auto &p = dynamic_cast<const Chevron &>(comp);
        // 拷贝关键点位置
    ...
    // 序列化，并记录已操作的
    void Chevron::serialize(json &j, std::vector<std::string> &processed) {
        // 父类序列化
        Rectangle::serialize(j, processed);
    }
    // 反序列化
    ComponentAbstract &Chevron::operator=(const json &j) {
        // 父类反序列化
    ...
    //// Basics虚接口
    void Chevron::moveCorePoint(const std::string &id, const double &dx, const double &dy) {
    ...
    //// IsometricCube接口
    // 计算路径
    const std::string Chevron::getPath() const {
